<script lang="ts">
    ...
    interface Props {
        on_toggle_search: () => void;
        on_show_help: () => void;
        search_panel_open: boolean;
    }

    let { on_toggle_search, on_show_help, search_panel_open }: Props = $props();
</script>

<div class="flex items-center gap-2">
    <button
        class="flex cursor-pointer items-center justify-center rounded-md border border-transparent bg-transparent p-2 text-(--text-primary) transition-all duration-200 hover:border-(--border-hover) hover:bg-(--bg-secondary) active:scale-98 {search_panel_open
