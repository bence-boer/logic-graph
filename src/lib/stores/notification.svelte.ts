...
export interface NotificationPreset<
    ResultData,
    Payloads extends readonly unknown[] = readonly unknown[]
> {
...
export type NotificationPresetData = {
    [Key in keyof NotificationPresets]: NotificationPresets[Key] extends NotificationPreset<
        infer Data
    >
        ? Data
        : never;
...
): preset is NotificationPreset<ResultData> & {
    actions: (data: ResultData) => NotificationAction<Payloads>[];
} => {
...
export const notification_store = (() => {
    ...
    return {
         ...
         * @returns Notification ID
         */
        success<Payload>(message: string, options?: NotificationOptionData<Payload>): string {
         ...
         * @returns Notification ID
         */
        error<Payload>(message: string, options?: NotificationOptionData<Payload>): string {
         ...
         * @returns Notification ID
         */
        warning<Payload>(message: string, options?: NotificationOptionData<Payload>): string {
         ...
         * @returns Notification ID
         */
        info<Payload>(message: string, options?: NotificationOptionData<Payload>): string {
        ...
        success_for_command<NotificationPreset extends NotificationPresetID>(
            ...
            const preset = NOTIFICATION_PRESETS[command_id] as
            ...
            const message =
                typeof preset.message_template === 'function'
                    ? preset.message_template(
                          result_data || ({} as NotificationPresetData[NotificationPreset])
                      )
                    : preset.message_template;
        ...
        error_for_command(
            ...
            error: string,
            options?: Pick<NotificationOptionData<unknown>, 'actions'>
        ): string {
