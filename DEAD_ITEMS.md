...
Important: these are _candidates_ for removal. They were detected with name-based grep and module-graph analysis (madge). False positives are possible (dynamic imports, string-based usage, usage outside scanned folders). Please review each item before deleting. Do not rely solely on this list to delete code.
